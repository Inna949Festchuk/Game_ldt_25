...
8. Для обновления проекта из папки Game_ldt_25 запускаем перезапускаем контейнер с пересоборкой
```bash
git pull && docker-compose up -d --build
```
9. Останавливаем контейнер и удаляем !ВСЕ! старые контейнеры с выгрузкой томов, если у вас если что-то пошло совсем не так, далее ппереустанавливаем игру по пунктам выше:
```bash
docker-compose down && docker system prune -a --volumes --force
```
---
